...
class CategoryRepository  implements CategoryRepositoryInterface
{
    ...
    public function store(array $data): Model
    {
        ...
        for ($i = 0; $i < count(($data['locales'] ?? [])); $i++) {
            ...
            $translateRecord['category_id'] = $createdModel->id;
            $translateRecords[] = $translateRecord;

            $locale = $localeData['locale'];
        }
        foreach ($translateRecords as $translateRecord) {
            CategoryTranslation::create($translateRecord);
        }
        DB::commit();
        return $createdModel;
    }
    ...
    public function update(Model $model, array $data): Model
    {
        ...
        foreach (($data['locales'] ?? []) as $localeData) {
            ...
            $translateRecord =  $localeData;
            $translateRecord['category_id'] = $model->id;
            $oldTranslation = CategoryTranslation::where('category_id', $model->id)->where('locale', $locale)->first();
            if ($oldTranslation) {
                CategoryTranslation::where('id', $oldTranslation->id)->update($translateRecord);
            } else {
                CategoryTranslation::create($translateRecord);
            }
        }
    ...
    {
        $model->delete();
        $filePath = CATEGORY_IMAGE_NAME . '/' . $model->image;
    ...
    {
        $folderPath = CATEGORY_IMAGE_NAME;
        if ($oldFileName) {
