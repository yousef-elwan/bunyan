...
use App\Models\ReportType\ReportTypeTranslation;
...
class ReportTypeRepository implements ReportTypeRepositoryInterface
{
    ...
    public function __construct(
        private readonly ReportType $model,
    ) {}
    ...
    public function store(array $data): Model
    {
        ...
        for ($i = 0; $i < count(($data['locales'] ?? [])); $i++) {
            ...
            $translateRecord['report_type_id'] = $createdModel->id;
            $translateRecords[] = $translateRecord;

            $locale = $localeData['locale'];
        }
        foreach ($translateRecords as $translateRecord) {
            ReportTypeTranslation::create($translateRecord);
        }
        DB::commit();
        return $createdModel;
    }
    ...
    public function update(Model $model, array $data): Model
    {
        ...
        foreach (($data['locales'] ?? []) as $localeData) {
            ...
            $translateRecord =  $localeData;
            $translateRecord['report_type_id'] = $model->id;
            $oldTranslation = ReportTypeTranslation::where('report_type_id', $model->id)->where('locale', $locale)->first();
            if ($oldTranslation) {
                ReportTypeTranslation::where('id', $oldTranslation->id)->update($translateRecord);
            } else {
                ReportTypeTranslation::create($translateRecord);
            }
        }
