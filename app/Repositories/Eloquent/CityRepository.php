...
class CityRepository implements CityRepositoryInterface
{
    ...
    public function update(Model $model, array $data): Model
    {
        ...
        foreach (($data['locales'] ?? []) as $localeData) {
            ...
            $translateRecord =  $localeData;
            $translateRecord['city_id'] = $model->id;
            $oldTranslation = CityTranslation::where('city_id', $model->id)->where('locale', $locale)->first();
            if ($oldTranslation) {
                CityTranslation::where('id', $oldTranslation->id)->update($translateRecord);
            } else {
                CityTranslation::create($translateRecord);
            }
        }
