...
@section('title', __('dashboard/category.list.page_title'))
...
@push('css_or_js')
    <script>
        ...
        Object.assign(window.AppConfig.routes, {
            'dashboard.category.edit': "{{ route('dashboard.category.edit', ['category' => ':category']) }}",
            'api.category.destroy': "{{ route('api.category.destroy', ['category' => ':category']) }}",
            'api.category': "{{ route('api.category') }}",
        });
        Object.assign(window.AppConfig.i18n, @json(__('dashboard/category.list')), {
            'pagination_info': '{{ __('dashboard/category.list.pagination_info') }}',
            'edit_tooltip': '{{ __('dashboard/category.list.edit_tooltip') }}',
            'delete_tooltip': '{{ __('dashboard/category.list.delete_tooltip') }}',
            'cancel_button': '{{ __('dashboard/category.list.cancel_button') }}',
        });
    </script>
    @vite(['resources/js/alpine/dashboard/category/list.js'])
@endpush

@section('content')
    <div class="flex flex-col min-h-screen bg-page-bg" x-data="categoryList()">
        ...
        <div class="bg-card-bg shadow-sm rounded-lg mb-4 p-4">
            ...
            <div class="flex flex-wrap items-center justify-between gap-y-2">
                <h1 class="text-2xl font-bold text-text-primary">
                    {{ __('dashboard/category.list.page_title') }}
                </h1>
            </div>
        </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 p-4">
                ...
                <div class="flex-grow flex flex-wrap items-center gap-3">
                    ...
                    <button
                        ...
                        {{ __('dashboard/category.list.refresh_button') }}
                    </button>
                </div>
                ...
                <div class="flex-shrink-0">
                    <a href="{{ route('dashboard.category.create') }}"
                        ...
                        <span
                            class="hidden sm:inline font-medium text-sm">{{ __('dashboard/category.list.add_new_button') }}</span>
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="bg-card-bg rounded-xl shadow-custom border border-border-color">
            ...
            <div class="hidden overflow-x-auto md:block">
                <table class="w-full text-sm text-left" id="categoriesDataTable">
                    <thead class="text-xs uppercase bg-gray-50 text-text-secondary">
                        <tr>
                            <th class="p-4">{{ __('dashboard/category.list.image') }}</th>
                            <th class="p-4">{{ __('dashboard/category.list.name') }}</th>
                            <th class="p-4">{{ __('dashboard/category.list.addon') }}</th>
                            <th class="p-4 text-center">{{ __('dashboard/category.list.actions') }}</th>
                        </tr>
                    </thead>
            ...
            <!-- Mobile Cards -->
            <div id="categoryCardsContainer" class="p-4 space-y-4 md:hidden">
                <template x-if="isLoading">
                    <div class="flex items-center gap-4 p-4 bg-white border rounded-xl shadow-sm border-border-color animate-pulse"
            ...
            <div x-show="!isLoading && allData.length === 0" x-cloak class="py-10 text-center">
                <p class="text-text-secondary">{{ __('dashboard/category.list.no_data_found') }}</p>
            </div>
        </div>
        ...
        <div x-show="isActionSheetOpen" x-cloak class="fixed inset-0 z-30 flex items-end">
            ...
            <div x-show="isActionSheetOpen" x-transition:enter="transition ease-in-out duration-300 transform"
                ...
                <div class="space-y-2">
                    <template x-for="action in categoryActions" :key="action.label">
                ...
                <div class="mt-4">
                    <button @click="isActionSheetOpen = false"
                        class="w-full bg-card-bg p-3 text-lg rounded-lg font-semibold text-text-primary">{{ __('dashboard/category.list.cancel_button') }}</button>
                </div>
            </div>
        </div>
    </div>
