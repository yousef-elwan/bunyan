...
@section('title', __('dashboard/property.create.page_title'))
        ...
        <div class="bg-card-bg shadow-sm rounded-lg mb-4 p-4">
            ...
            <div class="flex flex-wrap items-center justify-between gap-y-2">
                <h1 class="text-2xl font-bold text-text-primary">
                    {{ __('dashboard/property.create.page_title') }}
                </h1>
            </div>
        </div>

        <form id="addPropertyForm" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
            @csrf
            ...
            <div class="lg:col-span-8 space-y-6">
                            ...
                            <div x-show="activeTab === '{{ $lang->locale }}'" x-cloak>
                                <div class="mb-5">
                                    <label for="propDesc_{{ $lang->locale }}"
                                        class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_description', ['lang' => strtoupper($lang->name)]) }}
                                        <span class="text-danger">*</span></label>
                                    <textarea id="propDesc_{{ $lang->locale }}" name="content_{{ $lang->locale }}" class="tinymce-editor"
                                        @if ($lang->direction === 'rtl') dir="rtl" @endif>{{ old('content_' . $lang->locale) }}</textarea>
                                    <div class="mt-1 text-sm text-danger" id="content_{{ $lang->locale }}_error"></div>
                                </div>
                                <div>
                                    <label for="propLocation_{{ $lang->locale }}"
                                        class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_location_desc', ['lang' => strtoupper($lang->name)]) }}
                                    </label>
                                    <input type="text" id="propLocation_{{ $lang->locale }}"
                                        ...
                                        value="{{ old('location_' . $lang->locale) }}">
                ...
                <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                    <h2 class="pb-4 mb-6 text-lg font-semibold border-b border-border-color text-text-primary">
                        {{ __('dashboard/property.create.section_location') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        <div class="space-y-6">
                            <div>
                                <label for="propCity"
                                    class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_city') }}
                                    <span class="text-danger">*</span></label>
                                <select id="propCity" name="city_id"
                                    class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-accent-primary focus:border-accent-primary"
                                    required>
                                    <option value="" selected disabled>
                                        {{ __('dashboard/property.create.select_city_placeholder') }}</option>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label for="propLatitude"
                                        class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_latitude') }}
                                        <span class="text-danger">*</span></label>
                                ...
                                <div>
                                    <label for="propLongitude"
                                        class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_longitude') }}
                                        <span class="text-danger">*</span></label>
                            ...
                            <div>
                                <label for="propVideoUrl"
                                    class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_video_url') }}</label>
                        ...
                        <div>
                            <label
                                class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_map') }}</label>
                ...
                <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                    <h2 class="pb-4 mb-6 text-lg font-semibold border-b border-border-color text-text-primary">
                        {{ __('dashboard/property.create.section_details') }}</h2>
                    <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
                        <div>
                            <label for="propArea"
                                class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_area') }}
                            </label>
                            <input type="number" id="propArea"
                                class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-accent-primary focus:border-accent-primary"
                                value="{{ old('size') }}">
                        ...
                        <div>
                            <label for="propYearBuilt"
                                class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_year_built') }}</label>
                            <select id="propYearBuilt" name="year_built"
                                class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-accent-primary focus:border-accent-primary">
                                <option value="">
                                    {{ __('dashboard/property.create.select_year_placeholder') }}
                                </option>
                                @for ($year = date('Y'); $year >= 1800; $year--)
                                    <option value="{{ $year }}" @selected(old('year_built') == $year)>
                                        {{ $year }}
                                    </option>
                                @endfor
                            </select>
                        ...
                        <div>
                            <label for="propFloor"
                                class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_floor') }}</label>
                            <select id="propFloor" name="floor_id"
                                class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-accent-primary focus:border-accent-primary">
                                <option value="">
                                    {{ __('dashboard/property.create.select_floor_placeholder') }}
                                </option>
                                @foreach ($floors as $floor)
                                    <option value="{{ $floor['id'] }}" @selected(old('floor_id') == $floor['id'])>{{ $floor['name'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label for="propBedrooms"
                                class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_rooms') }}</label>
                            <input type="number" id="propBedrooms" name="rooms_count"
                                class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-accent-primary focus:border-accent-primary"
                                min="0" value="{{ old('rooms_count') }}">
                            <div class="mt-1 text-sm text-danger invalid-feedback" id="rooms_count_error"></div>
                        </div>
                        <div>
                            <label for="propOrientation"
                                class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_orientation') }}</label>
                            <select id="propOrientation" name="orientation_id"
                                class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-accent-primary focus:border-accent-primary">
                                <option value="">
                                    {{ __('dashboard/property.create.select_orientation_placeholder') }}
                                </option>
                                @foreach ($orientations as $orientation)
                                    <option value="{{ $orientation['id'] }}" @selected(old('orientation_id') == $orientation['id'])>
                ...
                <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color"
                    id="customAttributesSection" style="display: none;">
                    <h2 class="pb-4 mb-6 text-lg font-semibold border-b border-border-color text-text-primary">
                        {{ __('dashboard/property.create.section_custom_attributes') }}</h2>
                ...
                <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                    <h2 class="pb-4 mb-6 text-lg font-semibold border-b border-border-color text-text-primary">
                        {{ __('dashboard/property.create.section_amenities') }}</h2>
                    <div>
                        <label
                            class="block mb-3 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_amenities') }}</label>
                            ...
                            @foreach ($amenities as $amenity)
                                <label class="flex items-center gap-2 font-normal cursor-pointer">
                                    <input type="checkbox" name="amenities[]" value="{{ $amenity['id'] }}"
                                        ...
                                        @checked(is_array(old('amenities')) && in_array($amenity['id'], old('amenities')))>
                ...
                <div class="sticky top-6 space-y-6">
                    ...
                    <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                        <h3 class="pb-4 mb-4 text-lg font-semibold border-b border-border-color text-text-primary">
                            {{ __('dashboard/property.create.section_publish') }}</h3>
                        <div class="space-y-3">
                            <button type="submit" id="savePropertyBtn"
                                class="w-full px-6 py-2.5 text-sm font-medium text-white rounded-lg bg-accent-primary hover:bg-opacity-90 disabled:opacity-50">{{ __('dashboard/property.create.button_save') }}</button>
                            <a href="{{ route('dashboard.properties.index') }}" id="cancelAddPropertyBtn"
                                class="block w-full text-center px-6 py-2 text-sm font-medium border rounded-lg border-border-color text-text-secondary hover:bg-page-bg">{{ __('dashboard/property.create.button_cancel') }}</a>
                        </div>
                    </div>
                    ...
                    <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                        <h3 class="pb-4 mb-4 text-lg font-semibold border-b border-border-color text-text-primary">
                            {{ __('dashboard/property.create.section_categorization') }}</h3>
                        <div class="space-y-4">
                            <div>
                                <label for="propStatus"
                                    class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_status') }}
                                    <span class="text-danger">*</span></label>
                                <select id="propStatus" name="type_id"
                                    class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-accent-primary focus:border-accent-primary"
                                    required>
                                    <option value="" selected disabled>
                                        {{ __('dashboard/property.create.select_status_placeholder') }}</option>
                            ...
                            <div>
                                <label for="propCategory"
                                    class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_category') }}
                                    <span class="text-danger">*</span></label>
                                <select id="propCategory" name="category_id"
                                    class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-accent-primary focus:border-accent-primary"
                                    required>
                                    <option value="" selected disabled>
                                        {{ __('dashboard/property.create.select_category_placeholder') }}</option>
                            ...
                            <div>
                                <label for="available_from"
                                    class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_available_from') }}
                                    <span class="text-danger">*</span></label>
                    ...
                    <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                        <h3 class="pb-4 mb-4 text-lg font-semibold border-b border-border-color text-text-primary">
                            {{ __('dashboard/property.create.label_price') }}</h3>
                        <div>
                            <label for="propPrice"
                                class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/property.create.label_price') }}
                                <span class="text-danger">*</span></label>
                            <input type="text" id="propPrice"
                                class="block w-full text-sm border-gray-300 rounded-md shadow-sm focus:ring-accent-primary focus:border-accent-primary price-input @if (old('price_on_request')) bg-gray-100 @endif"
                                data-hidden-target="#price_clean" value="{{ old('price') }}"
                                @if (old('price_on_request')) disabled @else required @endif>
                            <input type="hidden" name="price" id="price_clean" value="{{ old('price') }}">
                        ...
                        <div class="mt-3">
                            <label for="propPriceOnRequest" class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" id="propPriceOnRequest" name="price_on_request" value="true"
                                    class="w-4 h-4 rounded text-accent-primary border-gray-300 focus:ring-accent-primary"
                                    @checked(old('price_on_request'))>
                                <span
                                    class="text-sm text-text-secondary">{{ __('dashboard/property.create.label_price_on_request') }}</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div id="image-drop-area" class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                        <h2 class="pb-4 mb-6 text-lg font-semibold border-b border-border-color text-text-primary">
                            {{ __('dashboard/property.create.section_gallery') }}</h2>
                        <h3 class="mb-2 text-sm font-medium text-text-primary">
                            {{ __('dashboard/property.create.gallery_add_new') }}</h3>
                        <div>
                            <label for="propImagesUpload"
                                ...
                                {{ __('dashboard/property.create.label_upload_button') }}</label>
                            <input type="file" id="propImagesUpload" class="hidden" multiple accept="image/*">
                            <p class="mt-2 text-xs text-text-secondary">{{ __('dashboard/property.create.upload_note') }}
                            </p>
                        </div>
...
@push('script')
    ...
    <script>
        ...
        window.AppConfig.routes = {
            ...
            'api.dashboard-properties.images.store': "{{ route('api.dashboard-properties.images.store', ['property' => ':property']) }}",
            'api.dashboard-properties.store': "{{ route('api.dashboard-properties.store') }}",
            'dashboard.properties.index': "{{ route('dashboard.properties.index') }}",
        };
        window.AppConfig.i18n = @json(__('dashboard/property.create'));
        window.AppConfig.pageData = {
            locales: @json($langs->pluck('locale')->toArray())
        };
    ...
    @vite(['resources/js/alpine/dashboard/property/create.js'])
@endpush
