...
@section('title', __('dashboard/type.create.page_title'))
...
@push('css_or_js')
    <script>
        ...
        Object.assign(window.AppConfig.routes, {
            'api.type.store': "{{ route('api.type.store') }}",
            'dashboard.type.index': "{{ route('dashboard.type.index') }}",
        });
        Object.assign(window.AppConfig.i18n, @json(__('dashboard/type.create')));
        window.AppConfig.pageData.locales = @json($langs->pluck('locale')->toArray());
    </script>
    @vite(['resources/js/alpine/dashboard/type/create.js'])
@endpush
        ...
        <div class="bg-card-bg shadow-sm rounded-lg mb-4 p-4">
            ...
            <div class="flex flex-wrap items-center justify-between gap-y-2">
                <h1 class="text-2xl font-bold text-text-primary">
                    {{ __('dashboard/type.create.page_title') }}
                </h1>
            </div>
        </div>

        <form id="createTypeForm" class="space-y-6">
            @csrf
            <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                <h2 class="pb-4 mb-6 text-lg font-semibold border-b border-border-color text-text-primary">
                    {{ __('dashboard/type.create.primaryinfo') }}
                </h2>
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    @foreach ($langs as $lang)
                        <div class="mb-5">
                            <label for="name_{{ $lang->locale }}"
                                class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/type.create.entername') }}
            ...
            <div class="flex justify-end gap-3 pt-5">
                <a href="{{ route('dashboard.type.index') }}"
                    class="px-6 py-2 text-sm font-medium border rounded-lg border-border-color text-text-secondary hover:bg-page-bg">{{ __('dashboard/type.create.cancel') }}</a>
                <button type="submit" id="createTypeBtn"
                    class="px-6 py-2 text-sm font-medium text-white rounded-lg bg-accent-primary hover:bg-opacity-90 disabled:opacity-50">{{ __('dashboard/type.create.save') }}</button>
            </div>
        </form>
    </div>
@endsection
