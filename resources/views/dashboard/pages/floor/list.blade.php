...
@section('title', __('dashboard/floor.list.page_title'))
...
@push('css_or_js')
    ...
    <script>
        ...
        Object.assign(window.AppConfig.routes, {
            'dashboard.floor.edit': "{{ route('dashboard.floor.edit', ['floor' => ':floor']) }}",
            'api.floor.destroy': "{{ route('api.floor.destroy', ['floor' => ':floor']) }}",
            'api.floor': "{{ route('api.floor') }}",
        });
        Object.assign(window.AppConfig.i18n, @json(__('dashboard/floor.list')), {
            'pagination_info': '{{ __('dashboard/floor.list.pagination_info') }}',
            'edit_tooltip': '{{ __('dashboard/floor.list.edit_tooltip') }}',
            'delete_tooltip': '{{ __('dashboard/floor.list.delete_tooltip') }}',
            'cancel_button': '{{ __('dashboard/floor.list.cancel_button') }}',
        });
    </script>
    @vite(['resources/js/alpine/dashboard/floor/list.js'])
@endpush

@section('content')
    <div class="flex flex-col min-h-screen bg-page-bg" x-data="floorList()">
        ...
        <div class="bg-card-bg shadow-sm rounded-lg mb-4 p-4">
            ...
            <div class="flex flex-wrap items-center justify-between gap-y-2">
                <h1 class="text-2xl font-bold text-text-primary">
                    {{ __('dashboard/floor.list.page_title') }}
                </h1>
            </div>
        </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 p-4">
                ...
                <div class="flex-grow flex flex-wrap items-center gap-3">
                    ...
                    <button
                        ...
                        {{ __('dashboard/floor.list.refresh_button') }}
                    </button>
                </div>
                <!-- Right Side: Add New Button -->
                <div class="flex-shrink-0">
                    <a href="{{ route('dashboard.floor.create') }}"
                        ...
                        <span
                            class="hidden sm:inline font-medium text-sm">{{ __('dashboard/floor.list.add_new_button') }}</span>
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="bg-card-bg rounded-xl shadow-custom border border-border-color">
            ...
            <div class="hidden overflow-x-auto md:block">
                <table class="w-full text-sm text-left" id="floorsDataTable">
                    <thead class="text-xs uppercase bg-gray-50 text-text-secondary">
                        <tr>
                            <th class="p-4">{{ __('dashboard/floor.list.name') }}</th>
                            <th class="p-4">{{ __('dashboard/floor.list.value') }}</th>
                            <th class="p-4">{{ __('dashboard/floor.list.addon') }}</th>
                            <th class="p-4 text-center">{{ __('dashboard/floor.list.actions') }}</th>
                        </tr>
                    </thead>
            ...
            <!-- Mobile Cards -->
            <div id="floorCardsContainer" class="p-4 space-y-4 md:hidden">
                <template x-if="isLoading">
            ...
            <div x-show="!isLoading && allFloorData.length === 0" x-cloak class="py-10 text-center">
                <p class="text-text-secondary">{{ __('dashboard/floor.list.no_data_found') }}</p>
            </div>
        </div>
        ...
        <div x-show="isActionSheetOpen" x-cloak class="fixed inset-0 z-30 flex items-end">
            ...
            <div x-show="isActionSheetOpen" x-transition:enter="transition ease-in-out duration-300 transform"
                ...
                <div class="space-y-2">
                    <template x-for="action in floorActions" :key="action.label">
                ...
                <div class="mt-4">
                    <button @click="isActionSheetOpen = false"
                        class="w-full bg-card-bg p-3 text-lg rounded-lg font-semibold text-text-primary">{{ __('dashboard/floor.list.cancel_button') }}</button>
                </div>
            </div>
        </div>
    </div>
