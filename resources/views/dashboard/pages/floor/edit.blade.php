...
@section('title', __('dashboard/floor.edit.page_title'))
...
@push('css_or_js')
    <script>
        ...
        Object.assign(window.AppConfig.routes, {
            'api.floor.update': "{{ route('api.floor.update', ['floor' => $floor->id]) }}",
            'dashboard.floor.index': "{{ route('dashboard.floor.index') }}",
        });
        Object.assign(window.AppConfig.i18n, @json(__('dashboard/floor.edit')));
        window.AppConfig.pageData.locales = @json($langs->pluck('locale')->toArray());
    </script>
    @vite(['resources/js/alpine/dashboard/floor/edit.js'])
@endpush
        ...
        <div class="bg-card-bg shadow-sm rounded-lg mb-4 p-4">
            ...
            <div class="flex flex-wrap items-center justify-between gap-y-2">
                <h1 class="text-2xl font-bold text-text-primary">
                    {{ __('dashboard/floor.edit.page_title') }}
                </h1>
            </div>
        </div>

        <form id="editFloorForm" class="space-y-6">
            @csrf
            <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                <h2 class="pb-4 mb-6 text-lg font-semibold border-b border-border-color text-text-primary">
                    {{ __('dashboard/floor.edit.primaryinfo') }}</h2>
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    @foreach ($langs as $lang)
                        <div class="mb-5">
                            <label for="name_{{ $lang->locale }}"
                                class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/floor.edit.entername') }}
                            ...
                            <input type="text" id="name_{{ $lang->locale }}" name="name_{{ $lang->locale }}"
                                ...
                                value="{{ $floor->translations->where('locale', $lang->locale)->first()?->name ?? '' }}"
            ...
            <div class="p-5 bg-card-bg rounded-xl shadow-custom border border-border-color">
                <h2 class="pb-4 mb-6 text-lg font-semibold border-b border-border-color text-text-primary">
                    {{ __('dashboard/floor.edit.details') }}</h2>
                <div>
                    <label for="value"
                        class="block mb-2 text-sm font-medium text-text-primary">{{ __('dashboard/floor.edit.value') }}</label>
            ...
            <div class="flex justify-end gap-3 pt-5">
                <a href="{{ route('dashboard.floor.index') }}"
                    class="px-6 py-2 text-sm font-medium border rounded-lg border-border-color text-text-secondary hover:bg-page-bg">{{ __('dashboard/floor.edit.cancel') }}</a>
                <button type="submit" id="updateFloorBtn"
                    class="px-6 py-2 text-sm font-medium text-white rounded-lg bg-accent-primary hover:bg-opacity-90 disabled:opacity-50">{{ __('dashboard/floor.edit.save') }}</button>
            </div>
        </form>
    </div>
@endsection
