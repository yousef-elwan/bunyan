    ...
    return {
            ...
            try {
                const params = new URLSearchParams({
                    ...
                    filters: btoa(JSON.stringify([
                        { id: 'is_blacklist', filterFns: 'equals', value: true }
                    ]))
                });
        ...
        renderTableAndCards() {
            ...
            this.allPropertiesData.forEach(property => {
                ...
                const blacklistReason = property.blacklist_reason || 'N/A';
                ...
                row.innerHTML = `
                    ...
                    <td class="p-3">
                        ${ownerIsMe ? translate('owner_is_me') : `
                    ...
                    <td class="p-3">${blacklistReason}</td>
                    <td class="p-3">
                        <button class="px-3 py-1 text-sm font-medium text-white bg-danger rounded-lg hover:bg-opacity-90 remove-blacklist-action"
                                data-id="${property.id}">
                            <i class="fas fa-ban mr-1"></i> ${translate('remove_from_blacklist')}
                        </button>
                    </td>
                `;
                    ...
                    <div class="flex gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-sm text-text-secondary mt-1">
                                <span class="font-medium">${translate('th_blacklist_reason')}:</span> ${blacklistReason}
                            </p>
                            <div class="mt-3">
                                <button class="w-full px-3 py-2 text-sm font-medium text-white bg-danger rounded-lg hover:bg-opacity-90 remove-blacklist-action"
                                        data-id="${property.id}">
                                    <i class="fas fa-ban mr-1"></i> ${translate('remove_from_blacklist')}
                                </button>
                            </div>
            ...
            // إضافة مستمعين الأحداث للأزرار الجديدة
        ...
        setupEventListeners() {
            const self = this;
            document.querySelectorAll('.remove-blacklist-action').forEach(button => {
                button.addEventListener('click', function () {
                    const propertyId = this.getAttribute('data-id');
                    self.removeFromBlacklist(propertyId);
                });
            });
        },
        ...
        removeFromBlacklist(id) {
            Swal.fire({
                title: translate('confirm_remove_blacklist_title'),
                text: translate('confirm_remove_blacklist_text'),
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    try {
                        const url = getRoute('api.properties.toggle-blacklist', { property: id });
                        await api.post(url);

                        Swal.fire(
                            translate('remove_blacklist_success_title'),
                            translate('remove_blacklist_success_text'),
                            'success'
                        );

                        // إعادة تحميل البيانات بعد الإزالة
                        this.fetchProperties(this.currentPage);
                    } catch (error) {
                        console.error("Failed to remove from blacklist:", error);
                        Swal.fire('Error', 'Failed to remove property from blacklist', 'error');
                    }
                }
            });
        }
