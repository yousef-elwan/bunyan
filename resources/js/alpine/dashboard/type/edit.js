...
function setupEditForm() {
    ...
    const saveBtn = document.getElementById('updateTypeBtn');
    ...
    editForm.addEventListener('submit', async function (event) {
        ...
        try {
            ...
            console.log(getRoute('api.type.update'));
            
            await api.post(getRoute('api.type.update'), payload);

            Swal.fire({
                icon: 'success', title: translate('update_success_title'), text: translate('update_success_text'),
                timer: 2000, showConfirmButton: false
            }).then(() => window.location.href = getRoute('dashboard.type.index'));
