...
function setupCreateForm() {
    const createForm = document.getElementById('createConditionForm');
    if (!createForm) return;

    const saveBtn = document.getElementById('createConditionBtn');
    ...
    createForm.addEventListener('submit', async function (event) {
        ...
        try {
            ...
            await api.post(getRoute('api.condition.store'), payload);

            Swal.fire({
                icon: 'success', title: translate('create_success_title'), text: translate('create_success_text'),
                timer: 2000, showConfirmButton: false
            }).then(() => window.location.href = getRoute('dashboard.condition.index'));
