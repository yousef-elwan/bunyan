...
function setupCreateForm() {
    const createForm = document.getElementById('createOrientationForm');
    if (!createForm) return;

    const saveBtn = document.getElementById('createOrientationBtn');
    ...
    createForm.addEventListener('submit', async function (event) {
        ...
        try {
            ...
            await api.post(getRoute('api.orientation.store'), payload);

            Swal.fire({
                icon: 'success', title: translate('create_success_title'), text: translate('create_success_text'),
                timer: 2000, showConfirmButton: false
            }).then(() => window.location.href = getRoute('dashboard.orientation.index'));
